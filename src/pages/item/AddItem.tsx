...
export default function AddItem() {
  ...
  const { setItemList, setItem, setShowAdd, setIsSearchItemCopy } =
    useItemStore();
  ...
  const { mutate, isPending, error } = useMutation({
    ...
    mutationFn: async (data: TItem) => {
      return await axios.post("/Item/AddItem", data);
    },
    onSuccess: (res, variable: TItem) => {
      variable.itemID = res.data;
      setItemList(new Array(variable));
      setItem(variable);
      handleClose();
    },
  });
